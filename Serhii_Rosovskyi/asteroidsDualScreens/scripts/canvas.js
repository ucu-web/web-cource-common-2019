...
function configurations() {
  ...
  for (let i = 0; i < canv.length; i++) {
    canv[i].style.background =
      "url('https://i.ytimg.com/vi/VuWJ14R5k90/maxresdefault.jpg') no-repeat center fixed";
...
const randomCoordinates = (canv, radius) => ({
  x: clamp(radius, canv.width - radius * 2, Math.random() * canv.width),
  y: clamp(radius, canv.height - radius * 2, Math.random() * canv.height)
});
...
class Canvas {
  ...
  keyDown(ev) {
    ...
    if (this.canvas === canvases[0]) {
      ev.key === "w"
        ? (this.spaceShip.moveing = true)
        : ev.key === "a"
        ? (this.spaceShip.rotCoef = 1)
        : ev.key === "d"
        ? (this.spaceShip.rotCoef = -1)
        : ev.key === "s"
        ? this.spaceShip.fire(this.timeRunning)
        : null;
    } else {
      ev.key === "ArrowUp"
        ? (this.spaceShip.moveing = true)
  ...
  keyUp(ev) {
    ...
    if (this.canvas === canvases[0]) {
      ev.key === "w"
        ? (this.spaceShip.moveing = false)
        : ev.key === "a" || ev.key === "d"
        ? (this.spaceShip.rotCoef = 0)
        : null;
    } else {
      ev.key === "ArrowUp"
        ? (this.spaceShip.moveing = false)
  ...
  createAsteroid() {
    ...
    do {
      ...
      distance(this.spaceShip.x, this.spaceShip.y, coord["x"], coord["y"]) < 200
    );
    let accelerationX = (Math.random() - 0.5) * 5;
    let accelerationY = (Math.random() - 0.5) * 5;
    return new Asteroid(
      this.ctx,
      coord["x"],
      coord["y"],
      ...
      for (let j = 0; j < this.spaceShip.bullets.length; j++) {
        if (
          distance(
            this.asteroids[i].x,
            this.asteroids[i].y,
            this.spaceShip.bullets[j].x,
            this.spaceShip.bullets[j].y
          ) <
          this.asteroids[i].radius + this.bulletSize
        ) {

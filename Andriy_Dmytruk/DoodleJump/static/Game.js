...
export default class Game {
  ...
  constructor(container) {
    ...
    this.doodle = new Doodle({
      x: (this.width - Doodle.width) / 2,
      y: (this.height - Doodle.height) / 2
    });
  ...
  render(duration) {
    ...
    this.platforms = this.platforms.map(p =>
      getNewPlatformBasedOnDuration(p, duration, this.width)
    );
    this.bullets = this.bullets.map(b =>
      getNewBulletBasedOnDuration(b, duration)
    );
  ...
  renderAnimationFrame() {
    const render = time => {
      ...
      if (duration < 1) this.render(duration);

function randomizedQuickSort(array, comparisonFunction) {
    randomizedQuickSortWithBoundaries(array, comparisonFunction, 0, array.length - 1);
}

function randomizedQuickSortWithBoundaries(array, comparisonFunction, start, end) {
    if (start < end) {
        let correctPlacePosition = partition(array, comparisonFunction, start, end);

        randomizedQuickSortWithBoundaries(array, comparisonFunction, start, correctPlacePosition - 1);
        randomizedQuickSortWithBoundaries(array, comparisonFunction, correctPlacePosition + 1, end);
    }
}

function partition(array, comparisonFunction, start, end) {
    swap(array, end, (Math.random() * (end - start + 1) ^ 0) + start);
    let bound = start - 1, pivot = array[end];

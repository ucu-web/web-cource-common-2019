...
Blade.prototype.checkForSlice = function(fruit) {
    ...
    let length = this.swipes.length; // length of sword

    let stroke1 = this.swipes[length - 1]; // latest stroke
    let stroke2 = this.swipes[length - 2]; // second-to-latest stroke

    /* calculate distance from strokes 1 & 2 from fruit */
    let d1 = dist(stroke1.x, stroke1.y, fruit.position.x, fruit.position.y);
    let d2 = dist(stroke2.x, stroke2.y, fruit.position.x, fruit.position.y);

    /* calculate distance from stroke1 to stroke2 */
    let d3 = dist(stroke1.x, stroke1.y, stroke2.x, stroke2.y);

    let sliced = d1 < fruit.size || 			// if stroke1 lands directly on the fruit
...
Blade.prototype.draw = function() {
    ...
    for (let i = 0; i < length; i++) {
        ...
        let s = map(i, 0, length, 2, 20); // shrink dots
